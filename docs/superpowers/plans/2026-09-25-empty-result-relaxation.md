...
**Superseded (independent review, decision 10).** `set_config('enable_seqscan', 'off', true)` is transaction-local, which is no help when the caller already has a transaction open: the settings stayed on until the caller committed (and a released DBAL savepoint keeps `SET LOCAL`), and the caller's next trivial query went from 0.6 ms to 159 ms. The probe now has one `MATERIALIZED` CTE per leaf and `EXISTS (SELECT 1 FROM m<i>)`; a materialized CTE is planned for full retrieval, which gives the same GIN bitmap plans with default settings and no `set_config`. Best of 5, `EXPLAIN (ANALYZE)`, 200 000 rows, default settings before / after: `wireless mouse aluminum` 559 / 10.6 ms, `mouse kettle` 0.1 / 8.5, `in_stock = false` 94 / 12.3, `price < 5000` 9.9 / 10.2, 16 common words 531 / 34.9. `pg_trgm.word_similarity_threshold` is read before and put back after each statement for the same reason.
